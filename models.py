...
class Agendamentos(db.Model):
    ...
    divergencias = db.relationship('Divergencias', backref='agendamento', uselist=False)
    trocas = db.relationship('Trocas', backref='agendamento', uselist=False)
    devolucoes = db.relationship('Devolucoes', backref='agendamento', uselist=False)
    recebimento = db.relationship('Recebimento', backref='agendamento', uselist=False)

class Divergencias(db.Model):
    __tablename__ = 'divergencias'
    id = db.Column(db.Integer, db.ForeignKey('agendamento.id'), nullable=False, primary_key=True)
